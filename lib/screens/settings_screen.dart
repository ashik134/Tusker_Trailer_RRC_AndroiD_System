...
class _AuthorizationStatusCard extends StatelessWidget {
  ...
  @override
  Widget build(BuildContext context) {
    final isTrustRejected = controller.isDeviceTrustRejected;
    ...
    if (isAuthenticated) {
      ...
      statusLabel = 'TRUSTED & AUTHORIZED';
      statusDetail =
          'This device is registered and has an active authenticated session.';
      statusBg = AppColors.successSoft;
      statusBorder = AppColors.success.withValues(alpha: 0.35);
    } else if (isTrustRejected) {
      statusColor = AppColors.danger;
      statusIcon = Icons.block_rounded;
      statusLabel = 'DEVICE NOT AUTHORIZED';
      statusDetail =
          'This device is not in the PLC trusted-device registry. '
          'Provide the Device ID above to your system administrator.';
      statusBg = AppColors.dangerSoft;
      statusBorder = AppColors.danger.withValues(alpha: 0.35);
    } else if (isConnected) {
      statusColor = AppColors.info;
      statusIcon = Icons.hourglass_top_rounded;
      statusLabel = 'CONNECTED — PENDING AUTH';
      statusDetail =
          'Device is connected to PLC. Authorization will be verified on next login attempt.';
      statusBg = const Color(0xFF0D1F30);
      statusBorder = AppColors.info.withValues(alpha: 0.3);
    } else {
      statusColor = AppColors.disabled;
      statusIcon = Icons.device_unknown_rounded;
      statusLabel = 'NOT CONNECTED';
      statusDetail =
          'Connect to PLC 14 to verify trusted-device authorization status.';
      statusBg = const Color(0xFF0D1520);
      statusBorder = AppColors.border;
    }
